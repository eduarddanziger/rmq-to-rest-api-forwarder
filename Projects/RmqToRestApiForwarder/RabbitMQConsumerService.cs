    ...
    protected override async Task ExecuteAsync(CancellationToken cancellationToken)
    {
        ...
        while (!cancellationToken.IsCancellationRequested)
        {
            try
            {
                ...
                // Retry queue: messages sit here for TTL then are dead-lettered back to the primary queue
                ...
                // Initialize debouncers with configured window (once)
                _renderDebouncer ??= new DebounceWorker(
                    name: "VolumeRenderChanged",
                    window: _volumeDebounceWindow,
                    processMessageAsync: async (msg, ct) =>
                    {
                        _logger.LogInformation("Debouncing chosen VolumeRenderChanged message at {UpdateDate:o} to be PROCESSED",
                            msg.UpdateDate);
                ...
                _captureDebouncer ??= new DebounceWorker(
                    ...
                    processMessageAsync: async (msg, ct) =>
                    {
                        _logger.LogInformation("Debouncing chosen VolumeCaptureChanged message at {UpdateDate:o} to be PROCESSED.",
                            msg.UpdateDate);
                        await ProcessMessageAsync(msg, ct);
                    },
                    ignoreMessageAsync: (msg, ct) =>
                ...
                consumer.ReceivedAsync += async (_, deliveryEvent) =>
                {
                    try
                    {
                        ...
                        var deviceMessageTypeAsInt = eventMessage[MessageFields.DeviceMessageType]?.GetValue<int?>();
                        var updateDateAsString = eventMessage[MessageFields.UpdateDate]?.GetValue<string>();
                        ...
                        var httpRequest = eventMessage[MessageFields.HttpRequest]?.GetValue<string>();
                        var urlSuffix = eventMessage[MessageFields.UrlSuffix]?.GetValue<string>();
                        eventMessage.Remove(MessageFields.HttpRequest);
                        eventMessage.Remove(MessageFields.UrlSuffix);
                ...
                // Successful initialization, reset attempt counter and wait until cancellation
                connectRetryIndex = 0;
                _logger.LogInformation("Consumer successfully connected and started.");
            ...
            catch (Exception ex)
            {
                ...
                var nextConnectRetryDelay = connectRetryIndex < connectRetryDelays.Length ? connectRetryDelays[connectRetryIndex] : maxConnectRetryDelay;
                connectRetryIndex = Math.Min(connectRetryIndex + 1, connectRetryDelays.Length); // cap growth
                _logger.LogWarning(ex, "Startup/connect attempt failed. Will retry in {DelaySeconds}s", nextConnectRetryDelay.TotalSeconds);

                try
                {
                    await Task.Delay(nextConnectRetryDelay, cancellationToken);
                }
                catch (OperationCanceledException)
                {
                    // Service shutdown requested
